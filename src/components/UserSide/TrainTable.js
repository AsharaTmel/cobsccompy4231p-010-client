      ...
      try {
        ...
        const trainDataPromises = data.map(async (train) => {
          ...
          return {
            ...
            start_station: gpsData.start_location || 'N/A',  // Directly use start_location
          ...
          {trains.map((train) => (
            <tr key={train.train_id}>
              ...
              {train.stations.slice(1, 10).map((station, index) => (
                <td key={index}>{station.station || 'N/A'} - {station.timestamp ? new Date(station.timestamp).toLocaleString() : 'N/A'}</td>
              ))}
              {train.stations.length < 10 && Array.from({ length: 10 - train.stations.length }).map((_, index) => (
                <td key={`empty-${index}`}>N/A</td>
              ))}
              <td>{train.stations[train.stations.length - 1]?.station || 'N/A'}</td>
